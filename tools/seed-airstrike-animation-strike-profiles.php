...
$emit_sql = in_array('--sql', $argv, true);
...
if ($emit_sql) {
    echo "-- Seed missing Portable Airstrikes strike-keyed animation profiles.\n";
    echo "-- Generated by tools/seed-airstrike-animation-strike-profiles.php --sql.\n";
    echo "-- Safe to rerun: existing profile_key rows are left unchanged.\n\n";
    echo "INSERT INTO airstrike_animation_profiles\n";
    echo "  (profile_key, display_name, vehicle, draft_source_json, draft_source_sha256, draft_version, created_by, updated_by)\n";
    echo "VALUES\n";
    ...
    foreach (seed_airstrike_profiles() as $source) {
        ...
        $values[] = sprintf(
            "  ('%s', '%s', '%s', '%s', '%s', 1, NULL, NULL)",
            str_replace("'", "''", (string) $source['ProfileKey']),
            str_replace("'", "''", mb_substr((string) $source['DisplayName'], 0, 160)),
            str_replace("'", "''", mb_substr((string) $source['Vehicle'], 0, 40)),
            str_replace("'", "''", $source_json),
            hash('sha256', $source_json)
        );
    }
