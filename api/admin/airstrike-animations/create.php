...
try {
    ...
    $profile = raidlands_airstrike_animations_create($source);
    if ($agent_proposal !== null) {
        raidlands_airstrike_agent_update_proposal((int) $agent_proposal['id'], 'saved');
        raidlands_admin_audit('airstrike_animation_create_agent_attribution', 'airstrike_animation_profile', (string) ($source['ProfileKey'] ?? ''), [
