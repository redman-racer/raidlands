...
describe("monument quality policy", () => {
  ...
  it("applies the requested tier caps and total resource budgets", () => {
    ...
    expect(resolveMonumentQuality("auto", "medium")).toMatchObject({ activeCloseLimit: 1, activeMidLimit: 9, activeMapLimit: 36, triangleBudget: 4_000_000, drawCallBudget: 3_000 });
    expect(resolveMonumentQuality("auto", "high")).toMatchObject({ activeCloseLimit: 2, activeMidLimit: 16, activeMapLimit: 64, triangleBudget: 5_000_000, drawCallBudget: 3_500 });
