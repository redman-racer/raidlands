...
export const MILITARY_BASE_COMPOSITE_ASSETS: MilitaryBaseCompositeAsset[] = [
  ...
  },
  {
    ...
    placements: [
      { x: -25, y: 0, z: 17, rotationY: Math.PI * 0.5 },
      { x: -25, y: 0, z: 7, rotationY: Math.PI * 0.5 },
    ],
  },
  {
    ...
    placements: [
      { x: -14, y: 0, z: 25, rotationY: 0 },
      { x: -8, y: 0, z: 25, rotationY: 0 },
      { x: 25, y: 0, z: -17, rotationY: Math.PI * 0.5 },
    ],
  },
  {
    ...
    placements: [
      { x: -18, y: 0, z: -7, rotationY: Math.PI * 0.35 },
      { x: -12, y: 0, z: -10, rotationY: Math.PI * 0.35 },
      { x: -6, y: 0, z: -13, rotationY: Math.PI * 0.35 },
      { x: 19, y: 0, z: 19, rotationY: Math.PI * 0.5 },
    ],
  },
  {
    id: "generator",
    ...
    placements: [
      { x: 23, y: 0, z: 12, rotationY: Math.PI * 0.5 },
      { x: 23, y: 0, z: 7, rotationY: Math.PI * 0.5 },
    ],
  },
  {
    id: "fuel-tank",
    path: "media/models/military-base/fuel_tank_a_600.glb",
    sha256: "81aba1652fba2426e63c143fadf28b434e9b8611c716d71f58c26256b912ec10",
    placements: [
      { x: 28, y: 0, z: 11, rotationY: 0 },
      { x: 28, y: 0, z: 4, rotationY: 0 },
    ],
  },
];

type MlrsPlacement = { x: number; y: number; z: number; rotationY: number; scale: number };

const MILITARY_BASE_MLRS_PLACEMENTS: Record<string, MlrsPlacement> = {
  desert_military_base_a: { x: -7, y: 0.35, z: -5, rotationY: Math.PI * 0.32, scale: 1.35 },
  desert_military_base_b: { x: -7, y: 0.35, z: -5, rotationY: Math.PI * 0.32, scale: 1.35 },
  desert_military_base_c: { x: -7, y: 0.35, z: -5, rotationY: Math.PI * 0.32, scale: 1.35 },
  desert_military_base_d: { x: -7, y: 0.35, z: -5, rotationY: Math.PI * 0.32, scale: 1.35 },
};
...
export function attachMilitaryBaseMlrs(monument: Group, mlrsSource: Group, prefab: string): Group | null {
  ...
  mlrs.scale.setScalar(placement.scale);
  mlrs.userData.raidlandsVehicleKind = "mlrs";
  mlrs.userData.raidlandsAnimationReady = true;
  mlrs.userData.raidlandsResidentVehicle = true;
  monument.userData.raidlandsResidentMlrs = true;

  mlrs.traverse((object) => {
    const renderable = object as Object3D & { isMesh?: boolean; frustumCulled?: boolean; renderOrder?: number };
    if (!renderable.isMesh) return;
    renderable.visible = true;
    renderable.frustumCulled = false;
    renderable.renderOrder = 4;
  });
